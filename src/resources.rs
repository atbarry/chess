...
use crate::{
    board::{BChange, Board, BoardPos, Piece},
    constants::{DESTROY_COLOR, MOVE_COLOR, PROMOTE_COLOR, PUSH_PREMOTE_COLOR, SWAP_COLOR},
};
...
impl Plugin for ResourcesPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(SelectedSquare::default())
            .insert_resource(HiglightedSquares {
                squares: Vec::new(),
            })
    ...
    pub fn from_board_changes(board: &Board, board_changes: Vec<BChange>) -> Self {
        ...
        for change in board_changes {
            let highlight_info = match change {
                ...
                BChange::MoveDestroy { end, .. } => (board.get_tile_entity(end), DESTROY_COLOR),
                BChange::BothMove { start2, .. } => (board.get_tile_entity(start2), SWAP_COLOR),
                BChange::Promotion { end, .. } => (board.get_tile_entity(end), PROMOTE_COLOR),
